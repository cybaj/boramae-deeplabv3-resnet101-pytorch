...
class ANGIODataset(data.Dataset):
    def __init__(self, mode='train', index=0, inference_target=['all'], valid_target='all', transform = None, logdir = None, infer_target_dir = None):
        from dataset import metadata, hrhcase_metadata, get_dataset, get_testset, get_trainsubset, get_inferenceset
        ...
        if mode == 'train':
            ...
            if 'all' in inference_target:
                self.trainset = get_testset()
            else:
                self.trainset = get_inferenceset(inference_target)
        ...
        if not mode == 'infer':
            ...
            for target in tqdm(self.trainset):
                ...
                filelist_path = os.path.join(target_root, f'{name}_filelist.txt')
                if mode == 'inference':
                    filelist_path = os.path.join(target_root, f'{name}_inference_filelist.txt')
