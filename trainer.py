...
class Trainer(object):
    ...
    def evaluate(self):
        ...
        ax.set_title('Deeplab V3+ Receiver Operating Characteristic curve')
        ...
        ax.set_title('Deeplab V3+ Precision Recall curve')
        ax.legend(loc="lower right")
    ...
    def inference(self):
        ...
        label_trues, label_preds = [], []
        for batch_idx, (data, target, filename) in tqdm.tqdm(
                takewhile(lambda x: x[0] < 9, enumerate(self.val_loader)), total=9,
                desc='Valid iteration=%d' % self.iteration, ncols=80,
                leave=False):
            if self.cuda:
                data, target = data.cuda(), target.cuda()
        ...
        if not osp.exists(out):
            os.makedirs(out)
        out_file = osp.join(out, 'inference_%012d.jpg' % self.iteration)
        skimage.io.imsave(out_file, fcn.utils.get_tile_image(visualizations))
        """
